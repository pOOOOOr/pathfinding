...
void naive_path (struct map *map, int x0, int y0, int x1, int y1);
void print_naive_path (struct map *map, int x0, int y0, int x1, int y1);
void init_curses (void);
void end_curses (void);
void curses_draw_map (struct map *map);
int heuristic_cost(struct map *map, int start, int goal);

int
heuristic_cost(struct map *map, int start, int goal)
{
    int w = map->width;
    return abs(goal % w - start % w) + abs(goal / w - start / w);
}
...
make_path (struct map *map, int x0, int y0, int x1, int y1)
{
    ...
    map->grid[start].heur = heuristic_cost(map, start, end);
    pq_enqueue (pq, start, map->grid[start].heur);

    int index, score, found = 0;
    int j, n_index, n_dist, n_heur;

    while (pq->size != 0)
    {
        pq_dequeue (pq, &index, &score);
        ...
        {
            n_index = neighbors[j];
            n_dist = map->grid[index].dist + map->grid[n_index].cost;
            n_heur = heuristic_cost(map, n_index, end);

            if (n_index == end)
            {
                map->grid[n_index].dist = n_dist;
                map->grid[n_index].heur = n_heur;
                    ...
                    && !(map->grid[n_index].flags & SQ_FLAG_IMPASSABLE))
            {
                ...
                map->grid[n_index].heur = n_heur;
                map->grid[n_index].pre = index;
                pq_enqueue (pq, n_index, n_dist + n_heur);
            }
        }
